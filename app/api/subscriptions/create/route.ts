...
import { createProPlanCheckout } from "@/lib/services/picpay-billing-service";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await createProPlanCheckout({
      profileId: user.id,
      customerName,
      customerEmail,
      customerDocument,
    ...
    const message =
      ...
      typeof error.message === "string"
        ? error.message
        : "Nao foi possivel iniciar o plano Pro agora.";
